"""macOS-Notifications + Fehler-Mail (portiert aus dem iCloud-Sync-Projekt).

Notifications primär über ``rumps.notification`` (funktioniert nur innerhalb einer
laufenden rumps-App mit gültiger Bundle-ID). Fällt das aus — z. B. beim Start außerhalb
eines ``.app``-Bundles während der Entwicklung — wird ``pync`` (terminal-notifier) als
Fallback genutzt.

Mailversand bewusst per **einfachem SMTP an ein lokales Relay** (MailRelay-Projekt,
Default ``127.0.0.1:2525``), das selbst Upstream-Auth, STARTTLS und Retry/Backoff
übernimmt — daher kein Auth/TLS/Keychain-Passwort hier.
"""
...
def send_mail(host: str, port: int, sender: str, recipient: str, subject: str,
    ...
    Benachrichtigung darf den Betrieb nie beeinflussen). Liefert ``True`` bei Erfolg.
    """
    try:
        import smtplib
